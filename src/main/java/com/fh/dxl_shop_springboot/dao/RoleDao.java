...
public interface RoleDao {
    ...
    @Insert("insert into dxl_user_role (pid,name,url,icon,type,isDel,createDate,author) value (#{pid},#{name},#{url},#{icon},#{type},#{isDel},#{createDate},#{author})")
    void saveRoleData(UserRole userRole);

    @Select("select * from dxl_user_role where id=#{id}")
    UserRole selectRoleById(Integer id);

    @Update("update dxl_user_role set name=#{name},url=#{url},icon=#{icon},type=#{type},isDel=#{isDel} where id=#{id}")
